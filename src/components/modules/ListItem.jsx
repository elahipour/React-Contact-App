...
function ListItem({ contact, setSelectedContacts, selectedContacts, isShowChecks }) {
  ...
  return (
    <label htmlFor={`labled-${contact.id}`}>
      <li className="flex rounded-md items-center justify-between bg-white font-[600] px-2 ">
        ...
        {!showMenu ? (
          <button className="grid place-items-center py-8 relative" onClick={handleMenu}><span className="bg-blue-300 rounded-full w-8 h-8 absolute right-0 top-4 ">...</span></button>
        ) : (
          <div className="flex gap-3 px-2 py-1 my-1 rounded bg-blue-200">
            <button
            ...
            <EditContact contact={currentContact} modal={modal} />
          </div>
        )}
      </li>
    </label>
